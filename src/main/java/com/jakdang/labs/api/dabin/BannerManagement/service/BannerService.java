...
public class BannerService {
    ...
    public BannerResponseDto createBanner(MultipartFile file, Integer userIndex) throws IOException {
        ...
        banner.setBannerIsvisible(false);
        ...
        return new BannerResponseDto(
                ...
                savedBanner.getBannerIsvisible());
    }

    @Transactional
    public BannerResponseDto updateBanner(Integer bannerIndex, MultipartFile file, Integer userIndex)
            throws IOException {
        try {
            ...
            return new BannerResponseDto(
                    ...
                    updatedBanner.getBannerIsvisible());
    ...
    @Transactional
    public ResponseDTO<?> updateBannerIsvisible(List<Integer> bannerIndices) {
        try {
            for (Integer bannerIndex : bannerIndices) {
                Banner banner = bannerRepository.findById(bannerIndex)
                        .orElseThrow(() -> new RuntimeException("Banner not found"));
                banner.setBannerIsvisible(!banner.getBannerIsvisible());
                bannerRepository.save(banner);
            }
            log.info("배너 노출 여부 수정 성공");
            return ResponseDTO.createSuccessResponse("배너 노출 여부 수정 성공", null);
        } catch (Exception e) {
            return ResponseDTO.createErrorResponse(500, "배너 노출 여부 수정에 실패했습니다: " + e.getMessage());
        }
    }
}