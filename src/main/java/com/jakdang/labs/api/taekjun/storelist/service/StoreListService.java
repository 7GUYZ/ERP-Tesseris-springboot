package com.jakdang.labs.api.taekjun.storelist.service;
...
import com.jakdang.labs.api.taekjun.storelist.repository.StoreCategoryJtjRepo;
import com.jakdang.labs.api.taekjun.storelist.repository.AllStoreJtjRepo;
...
public class StoreListService {
    private final StoreCategoryJtjRepo categoryRepo;
    private final AllStoreJtjRepo storeRepo;

    public ResponseDTO<List<StoreCategoryDTO>> getStoreCategories(){
        ...
        return ResponseDTO.createSuccessResponse("가맹점 항목 불러오기 성공", categories.stream()
                .map(entity -> StoreCategoryDTO.builder()
    ...
    public ResponseDTO<List<StoreListDTO>> getFilteredStoreList(Integer store_category_index){
        List<Object[]> resultList = storeRepo.findFilteredStoreListWithUserIndex(store_category_index);

        List<StoreListDTO> dtoList = resultList.stream().map(arr -> 
    ...
    public ResponseDTO<?> getStoreDetail(Integer store_index){
        ...
        StoreDetailDTO detailDTO = StoreDetailDTO.builder()
