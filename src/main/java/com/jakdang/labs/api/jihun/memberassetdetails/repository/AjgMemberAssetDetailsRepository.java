...
@Repository
public interface AjgMemberAssetDetailsRepository extends JpaRepository<MemberAssetDetailsResponseDto, Integer> {
    ...
    @Query(value = """
        SELECT
            ...
            t6.user_id as suggestionUserId,
            t6.user_name as suggestionUserName,
            FORMAT(IFNULL(t7.total_cash, 0), '###,###.##') as temporaryStoreCashValue
        FROM user_tesseris u
        INNER JOIN user_cm c ON u.user_index = c.user_cm_index
        ...
        INNER JOIN suggestion_user t5 ON u.user_index = t5.recommendation_user_index
        INNER JOIN user_tesseris t6 ON t5.suggestion_user_index = t6.user_index
        LEFT JOIN (
        ...
        AND (:userPhone IS NULL OR u.user_phone LIKE CONCAT('%', :userPhone, '%'))
        AND (:userRoleIndex IS NULL OR u.user_role_index = :userRoleIndex)
        """, nativeQuery = true)
    Page<MemberAssetDetailsResponseDto> findMemberAssetDetails(
    ...
    @Query(value = """
        ...
        WHERE user_cm_index = (
            SELECT user_index FROM user_tesseris WHERE user_id = :userId
        )
        """, nativeQuery = true)
    @Modifying
    @Transactional
